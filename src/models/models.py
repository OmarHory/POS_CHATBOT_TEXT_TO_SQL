...
class OrderOption(Base):
    ...
    sku = Column(String(255))
    client_id = Column(Integer, ForeignKey("clients.id"), nullable=False)
    tax_exclusive_unit_price = Column(Float(precision=2))
    tax_exclusive_total_price = Column(Float(precision=2))
    tax_exclusive_discount_amount = Column(Float(precision=2))
    quantity = Column(Integer)
